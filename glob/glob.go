...
　　　如果 \ 不在 *、?、\ 之前，可以不用转义。
=============================================================================
转自：http://www.cnblogs.com/golove/p/3657362.html
============================================================================= */
...
以下面的模式为例，将它们以 * 为分隔符拆分成<子模式>，然后将子模式与字符串 str 进行匹配

“a?a*b?b*c?c” 拆分得到 a?a、b?b、c?c（将 a?a、b?b、c?c 中的 ? 当作普通字符串处理）
1、a?a 必须在 str 的<开头>出现，如果匹配成功，则删除 str 中匹配的部分，将剩下的字符串作为新
的 str 与后面的子模式进行匹配。
2、b?b 必须在 str 的<任意位置>出现一次，如果匹配成功，则删除 str 中匹配的部分及其之前的所有
内容，将剩下的字符串作为新的str与后面的子模式进行匹配。
3、c?c 必须在 str 的<尾部>出现，如果匹配成功，则整个字符串匹配成功。

“a*” 拆分得到 a
1、a 必须在 str 的<开头>出现，如果匹配成功，则整个字符串匹配成功。

“*a” 拆分得到 a
1、a 必须在 str 的<结尾>出现，如果匹配成功，则整个字符串匹配成功。

“a” 拆分得到 a
1、a 必须与 str <完全匹配>，如果匹配成功，则整个字符串匹配成功。
...
以下面的子模式为例，将它们以 ? 为分隔符拆分成<子表达式>，然后将子表达式与字符串 str 进行逐个匹配

“a??b” 拆分得到 a、?、?、b

如果 a??b 要匹配 str 的开头，则从 str 的开头与 a??b 进行正向匹配（函数：hasPrefix）。
如果 a??b 要匹配 str 的结尾，则从 str 的尾部与 a??b 进行逆向匹配（函数：hasSuffix）。
如果 a??b 要匹配 str 的中间，则从 str 的开头与 a??b 进行正向匹配，如果匹配失败，则将 str
的第一个字符去掉，再进行下一轮比较，直到匹配成功或 str 为空为止（函数：hasMidfix）。

总结：

对于整个模式字符串，拆分后将得到“子模式列表”，每个子模式又包含“子表达式列表”：

“a?a*b?b*c?c” 拆分后得到：
[[a、?、a]、[b、?、b]、[c、?、c]]

每个子模式或子表达式除了“内容”信息外，都有相应的“匹配方式”标记，以确定如何与 str 进行匹配。匹配
方式包括“匹配起始位置、匹配任意位置、匹配结束位置、完全匹配、匹配第一个字符（? 的情况）”
============================================================================= */
